"""Auto Subrogation claims table operations."""
...
TABLE_NAME = "dbo.tblAutoSubro_combinedOpenClaimsPredictions_Selector"
PRIMARY_KEY = "ID"
CLAIM_NUMBER_COLUMN = "CLM_NBR_U"
...
async def get_claims(
    ...
    try:
        ...
        logger.exception("Failed to fetch Auto Subrogation claims")
        raise HTTPException(
...
async def get_claim_by_number(claim_number: str) -> list[dict[str, Any]]:
    """Return every Auto Subrogation row for one claim number."""
    try:
        records = await fetch_records_async(
            TABLE_NAME,
        ...
        logger.exception("Failed to fetch Auto Subrogation claim %s", claim_number)
        raise HTTPException(
            status_code=500, detail={"error": "Database operation failed"}
        ) from error
...
async def upsert_claims(
    ...
    try:
        ...
        logger.exception("Failed to upsert Auto Subrogation claims")
        raise HTTPException(
            status_code=500, detail={"error": "Database operation failed"}
        ) from error
