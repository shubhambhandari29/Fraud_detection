"""Auto Subrogation claims routes."""
...
from services.auto_subrogation.claims_service import (
    get_claim_by_number,
    get_claims,
    upsert_claims,
)
...
@router.get("/")
async def get_auto_subrogation_claims(request: Request) -> list[dict[str, Any]]:
    return await get_claims(dict(request.query_params))


@router.get("/get/{claim_number}")
async def get_auto_subrogation_claim_by_number(
    claim_number: str,
) -> list[dict[str, Any]]:
...
async def upsert_auto_subrogation_claim(
